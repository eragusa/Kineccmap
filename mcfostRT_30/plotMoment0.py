...
CO.plot_map(subtract_cont = True,iTrans = 0, #psf_FWHM = 0.03,  
                 ...
                 moment = 0, Jy=True, per_beam=True, Delta_v = deltav)#,fmax = 1e-18)

imagedata=CO.last_im


plt.figure(3)
plt.pcolormesh(-xvec,yvec,imagedata,vmin=0.,vmax=0.4,cmap='inferno')#'gist_ncar')
plt.xlim([2.4,-2.4])
plt.ylim([-2.4,2.4])

plt.text(3.4, 2.7, r'Moment0 CO 3-2, Synthetic',color='white',fontsize=14)
#plt.text(3.4, 2.4, inclinstring,color='white',fontsize=14)
...
cb.set_label("Flux density [Jy$\\cdot$beam$^{-1}\\cdot$km s$^{-1}$]", size = 17)
...
ax.add_patch(beam)
plt.savefig('./COM0sim.png')

#pymcfost.CASA_simdata.pseudo_CASA_simdata(model,i=0,iaz=0,iTrans=None,simu_name = "pseudo_casa",beam=None,bmaj=None,bmin=None,bpa=None,subtract_cont=False,Delta_v=None)
...
hdul.writeto("M0_30" + ".fits", overwrite=True)
