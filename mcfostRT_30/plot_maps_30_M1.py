...
binarysemimaj=15.
distance=130
lorig=1.#0.2 #current binary size
...
vfield = 1.*hdu_list[0].data #1.* needed as the dataset appears to be big endian instead of little. 1.* changes it
#to check endianness vfield.dtype.byteorder
 #/ 1000 # m/s -> km/s
...
aout=10.7
incldisc=3.14/6.
velomax=30*np.sqrt(1./acav_u/binarysemimaj)*np.sin(incldisc) #30 km/s rescaled sqrt(1/R)
...
plt.pcolormesh(xvec,yvec,vfield, cmap='RdBu_r',vmin = -velomax*1.1, vmax=velomax*1.1)
...
file_path = './interpolate_RT_30_M1.pkl'
...
xgr2=hdu_list[0].data[0][0][0]
ygr2=hdu_list[0].data[1][0][0]
R=np.sqrt(xgr2**2+ygr2**2)
...
plt.figure(2)
plt.pcolormesh(xvec,yvec,height/R,vmin=0.0,vmax=0.4)
plt.colorbar()
ax=plt.gca()
ax.set_aspect('equal')
plt.xlim([-extent,extent])
...
plt.figure(3)
plt.pcolormesh(xvec,yvec,
                           vfield-interpolate_RT((xgr2,ygr2)), 
