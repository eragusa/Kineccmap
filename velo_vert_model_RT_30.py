...
rescale_v=29.6/np.sqrt(binarysemimaj) #km/s to rescale velocities to phys units compatible with scale lengths
...
corrector=1.#2.3#corrector for matching the last emission surface in units of H
corrector2=1.
...
xvcirc=np.array([x0,y0,zcirc*corrector2])*rescale_x
xvcircbottom=np.array([x0,y0,-zcirc*corrector2])*rescale_x
...
xvsim=np.array([xgrplan,ygrplan,H_RT*corrector2])*rescale_x
xvsimbottom=np.array([xgrplan,ygrplan,-H_RT*corrector2])*rescale_x
...
#velmin=-velmax
plt.figure(1)
plt.scatter(x1v[0,:],x1v[1,:],c=v1v[2,:]*matchsign,cmap="RdBu_r",vmin=velmin,vmax=velmax)
...
#velmin=-velmax
plt.figure(2)
plt.scatter(x1v[0,:],x1v[1,:],c=v1vpress[2,:]*matchsign,cmap="RdBu_r",vmin=velmin,vmax=velmax)
...
#velmin=-velmax
plt.figure(3)
plt.scatter(x1v[0,:],x1v[1,:],c=v1vsim[2,:]*matchsign,cmap="RdBu_r",vmin=velmin,vmax=velmax)
#plt.scatter(x1vbottom[0,:],x1vbottom[1,:],c=v1vpressbottom[2,:],cmap="seismic",vmin=velmin,vmax=velmax)
plt.colorbar()
plt.tricontour(x1v[0,:],x1v[1,:],v1vsim[2,:]*matchsign,levels=lev, linewidths=0.5, colors='k')
plt.axis('equal')
plt.xlim([-extent,extent])
plt.ylim([-extent,extent])
...
velmin=-velomax
plt.figure(6)
plt.scatter(x1v[0,:],x1v[1,:],c=(v1vsim[2,:]-v1circv[2,:])*matchsign,cmap="RdBu_r",vmin=velmin,vmax=velomax)
plt.colorbar()
...
velmin=-velomax
plt.figure(7)
plt.scatter(x1v[0,:],x1v[1,:],c=(-v1v[2,:]+v1vsim[2,:])*matchsign,cmap="RdBu_r",vmin=velmin,vmax=velomax)
plt.colorbar()
...
velmin=-velomax
plt.figure(8)
plt.scatter(x1v[0,:],x1v[1,:],c=(-v1vpress[2,:]+v1vsim[2,:])*matchsign,cmap="RdBu_r",vmin=velmin,vmax=velomax)
plt.colorbar()
...
file_path = './MCFOST/RT2A500/i_30_deg/interpolate_H_30.pkl'

# Open the file in binary read mode and load the function using pickle
with open(file_path, 'rb') as file:
    func_H2 = pickle.load(file)

#for a comparison with last emission layer, one needs to use the vertical height before rotation and rescale
